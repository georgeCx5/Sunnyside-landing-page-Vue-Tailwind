...
export default {
   ...
   computed: {
      hoverState() {
         return this.isHovered ? '#FFFFFF' : '#2C7566';
      }
   },
   mounted() {
      for (let i = 0; i < this.data.length; i++) {
         if (this.dataName == this.data[i].name) {
            this.svgHeight = this.data[i].height;
            this.dPath = this.data[i].d;
            return;
         }
      }
   }
...
<template>
   <button @mouseenter="isHovered = true" @mouseleave="isHovered = false">
      <svg width="20" :height="svgHeight" xmlns="http://www.w3.org/2000/svg">
         <path :d="dPath" :fill="hoverState" fill-rule="nonzero" />
      </svg>
   </button>
</template>